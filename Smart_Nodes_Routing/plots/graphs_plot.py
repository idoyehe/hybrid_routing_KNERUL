...
def plot_baselines_graphs(save_file, x_labels, y_data, oblivious_baseline, h_lines=None):
    bar_design = dict()
    bar_design["Non-Key Nodes Train Set"] = {"color": "darkseagreen", "hatch": '.', "alpha": 0.8}
    bar_design["Key Nodes Train Set"] = {"color": "palegreen", "hatch": 'x', "alpha": 0.8}
    ...
    plt.yticks(np.arange(bottom, 2, step=y_tick_offset * 3), fontsize=fontsize)
...
def plot_cross_topologies_graphs(save_file, topologies, y_data):
    bar_design = dict()
    bar_design["Uniform Link Weight\nInitialized RL"] = {"color": "darkseagreen", "hatch": '.', "alpha": 0.8}
    bar_design["Optimized Link Weight\nInitialization"] = {"color": "palegreen", "hatch": 'x', "alpha": 0.8}
    bar_design["Optimized Link Weight\nInitialized RL"] = {"color": "darkgreen", "fill": True, "hatch": None, "alpha": 1.0}
    fontsize = 11
    fig = plt.figure(figsize=(7.3, 5))
    ax = plt.subplot()
    ax.autoscale(enable=True)
    ...
    for y_label, value in y_data.items():
        ...
        ax.bar(ind + (offset * offset_value), y_data, width=width, align='center', label=y_label,yerr=yerr,
               ...
               color=bar_design[y_label]["color"], hatch=bar_design[y_label]["hatch"],
               alpha=bar_design[y_label]["alpha"])
        offset += 1
    ...
    ax.set_axisbelow(True)  # grid lines are behind the rest
    plt.ylim(bottom=1.0)
    plt.yticks(np.arange(1.0, 1.34, step=y_tick_offset *0.15), fontsize=fontsize)
    ax.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.2f'))
    plt.tight_layout()
    plt.savefig(save_file)
    plt.show()
...
if __name__ == "__main__":
    ...
    save_file = "{}_{}_baseline_{}.pdf".format(topology_name, traffic_name, baseline_name)
