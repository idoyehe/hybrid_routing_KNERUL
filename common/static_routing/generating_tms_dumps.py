...
def _dump_tms_and_opt(net: NetworkClass, url, matrix_sparsity: float, tm_type, initial_weights_clac,
    ...
    if initial_weights_clac:
        ...
        dst_splitting_ratios = traffic_distribution.reduce_src_dst_spr_to_dst_spr(net, optimal_src_dst_splitting_ratios)
        dst_mean_congestion = np.mean([traffic_distribution._calculating_traffic_distribution(dst_splitting_ratios, t[0])[0] for t in tms])
        print("Expected Congestion :{}".format(expected_objective))
    ...
    dict2dump = {
        ...
        "optimal_splitting_ratios": optimal_src_dst_splitting_ratios,
