...
if __name__ == "__main__":
    ...
    max_traffic_matrix = np.max(traffic_matrix_list, axis=0)
    max_congested_link, necessary_capacity, dst_splitting_ratio = optimal_load_balancing_LP_solver(net, max_traffic_matrix, return_spr=True)

    traffic_distribution = Optimizer_Abstract(net)
    max_tm_congestion = np.mean([traffic_distribution._calculating_traffic_distribution(dst_splitting_ratio, t)[0] for t in traffic_matrix_list])
    max_tm_congestion = np.round(max_tm_congestion, 4)
    print("Max Traffic Matrix routing scheme congestion: {}".format(max_tm_congestion))
