...
def mcf_QP_solver(net: NetworkClass, traffic_matrix_list):
    ...
    opt_ratio_value, splitting_ratios_vars_per_dest, r_vars_per_matrix, necessary_capacity_dict = _aux_mcf_QP_solver(
        net,
        traffic_matrix_list,
        gb_env)
    while True:
...
def _aux_mcf_QP_solver(net: NetworkClass, tm_list, gurobi_env, opt_ratio_value=None):
    ...
    try:
        logger.info("QP Submit to Solve {}".format(opt_qp_problem.ModelName))
        opt_qp_problem.update()
...
if __name__ == "__main__":
    ...
    for i, t_elem in enumerate(loaded_dict["tms"][0:l]):
        r_vars_per_matrix[i] = round(r_vars_per_matrix[i], 4)
        assert r_vars_per_matrix[i] >= t_elem[1] or error_bound(r_vars_per_matrix[i], t_elem[1])
        print("Matrix {}; multiple matrices congestion: {}; Optimal: {}; ratio: {}"
              .format(i, r_vars_per_matrix[i], t_elem[1], r_vars_per_matrix[i] / t_elem[1]))
