...
def optimal_load_balancing_finder(net: NetworkClass, traffic_matrix, opt_ratio_value=None):
    opt_lp_problem = Model(name="LP problem for optimal load balancing, given network and TM")
    ...
    for _arch in net_direct.edges:
        for flow in flows:
            ...
            g_var = opt_lp_problem.continuous_var(lb=0.0, name="g_{}_{}->{}".format(str(_arch), src, dst))
            arch_vars_per_flow[_arch][flow] = g_var
            arch_all_vars[_arch].append(g_var)

    if opt_ratio_value is None:
        opt_ratio = opt_lp_problem.continuous_var(lb=0.0, name="opt_ratio")
        opt_lp_problem.minimize(opt_ratio)
    else:
        opt_ratio = opt_ratio_value

    for _arch in net_direct.edges:
        _arch_capacity = net_direct.get_edge_key(_arch, key=EdgeConsts.CAPACITY_STR)
        opt_lp_problem.add_constraint(opt_lp_problem.sum(arch_all_vars[_arch]) <= _arch_capacity * opt_ratio)
    ...
    for flow in flows:
        ...
        for u in net_direct.nodes:
            ...
            from_some_u = opt_lp_problem.sum([arch_vars_per_flow[out_arch][flow] for out_arch in out_arches_dict[u]])
            to_some_u = opt_lp_problem.sum([arch_vars_per_flow[in_arch][flow] for in_arch in in_arches_dict[u]])
            opt_lp_problem.add_constraint(from_some_u == to_some_u)

    logger.info("LP Solving {}".format(opt_lp_problem.name))
    opt_lp_problem.solve()

    if opt_ratio_value is None:
        assert opt_lp_problem.get_solve_status().name == 'OPTIMAL_SOLUTION'
        opt_ratio = opt_ratio.solution_value

    if logger.level == logging.DEBUG:
        opt_lp_problem.print_solution()
    ...
    for _arch in net_direct.edges:
        for flow in flows:
            ...
            assert src != dst
            assert traffic_matrix[flow] > 0
            arch_vars_per_flow[_arch][flow] = arch_vars_per_flow[_arch][flow].solution_value

    opt_lp_problem.clear()
    ...
    return opt_ratio, link_carries_per_flow
