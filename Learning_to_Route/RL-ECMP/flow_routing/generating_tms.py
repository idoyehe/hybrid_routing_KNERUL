...
def _getOptions(args=argv[1:]):
    ...
    parser.add_argument("-n_e", "--network_elephant", type=float, help="The network elephant expectancy")
    parser.add_argument("-n_m", "--network_mice", type=float, help="The network mice expectancy")
...
def _generate_traffic_matrix_baseline(net: NetworkClass, matrix_sparsity: float, tm_type, elephant_percentage: float,
    ...
    for index in range(total_matrices):
        tm = one_sample_tm_base(graph=net,
                                ...
                                elephant_percentage=elephant_percentage, network_elephant=network_elephant,
                                network_mice=network_mice)
        opt_congestion,_ = get_optimal_load_balancing_flows_fractions(net, tm)  # heuristic flows splittings
...
if __name__ == "__main__":
    ...
    if args.tm_type == TMType.BIMODAL and args.network_elephant is None:
